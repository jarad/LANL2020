...
\section{USDA NIFA CAP}
\begin{frame}
\frametitle{USDA NIFA CAP}
  ...
  \begin{columns}
    \begin{column}{0.5\textwidth}
      \begin{itemize}
        ...
        \begin{itemize}
          \item Soil properties 2 functionals x 16 layers, 2 scalars.
          \item Climate dynamics 4 functionals x 365 daily values, 2 scalars.
          \item Land management practices, but mostly categorical scalars.
        \end{itemize}
      \end{itemize}
    \end{column}
...
%
%   \begin{itemize}
%       \item Small unstructured input vector and single scalar response \cite{currinBayesianPredictionDeterministic1991,sacksDesignAnalysisComputer1989}
...
\begin{frame}
  ...
  \framesubtitle{Extending Morris (2012)}

  \begin{columns}
    \begin{column}{0.5\textwidth}
      \begin{itemize}
        \item Let $Y \in \R$ be a scalar-valued output
        \item Let $\mathbf{X}$ be a $D$-sized input vector, $D \in \mathcal{N}$
      ...
      \end{aligh*}
        ...
        % \begin{equation*}
        %   \label{eq:weighted-distance}
        %   k(\mathbf{X_i}, \mathbf{X_j}) = \sigma^2_{ard} e^{-\frac{1}{2} \sum_{d = 1}^{D}{w_d (X_{i, d} - X_{j, d})^2}}
        % \end{equation*}
        % \newline
        % with $\sigma^2_{ard} > 0, w_d > 0 \ \forall \ d \in \{1, \dots, D\}$
      \end{itemize}
    \end{column}

    \begin{column}{0.5\textwidth}
      \begin{itemize}
        \item Let X model input that be a function of a continuous index $X_i(t), X_j(t)$ for $t \in [0, T], T \in \R^+$
      \end{itemize}

      \begin{equation*}
        \label{eq:weighted-distance-cts}
          D(X_i(t), X_j(t), w) = \int_{0}^{t}{ w(s - t) {(X_i(t) - X_j(t))}^2 ds}
      \end{equation*}


      \begin{align*}
        \mu_w         &\sim \pi(\mu_w)    \\
        \sigma_w      &\sim \pi(\sigma_w) \\
...
functional inputs.
    ...
    + 1}}$, where $D_1, \dots, D_M, D_{M + 1} \in \N$, be a $D = \sum_{m = 1}^{M} D_m + D_{M + 1}$ dimensional vector.
        \item $\mathbf{X} \in \R^{D_1} \times \cdots \times \R^{D_M} \times \R^{D_{M
    + 1}}$, where $D_1, \dots, D_M, D_{M + 1} \in \N$, be a
$D = \sum_{m = 1}^{M} D_m + D_{M + 1}$ dimensional vector
...
$f_Y \sim GP(\mu_Y,\sigma_Y)$
    ...
    \begin{column}{0.5\textwidth}
      ...
      \begin{tikzpicture}[
        ...
        \draw[myarrow, bend left=90] (XbarM) -- node [above] {          } (Y);
        \draw[draw=none            ] (X2)    -- node [     ] { $\vdots$ } (Xm);
        \draw[draw=none            ] (Xm)    -- node [     ] { $\vdots$ } (XM);
      \end{tikzpicture}
%
%       \begin{minipage}{0.85\textwidth}
%         \caption{
%           A directed acyclic graph depicting a deep Gaussian process.
