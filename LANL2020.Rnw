...
\begin{frame}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.5\textwidth}
      % \begin{itemize}
      %   \item If $\mathbf{X}$ is function of a continuous index $X_i(t), X_j(t)$ for $t \in [0, T], T \in \R^+$
      % \end{itemize}

      % If $\mathbf{X}$ is function of a continuous index $X_i(t), X_j(t)$ for $t \in [0, T], T \in \R^+$
