...
\includegraphics[height=0.2\textheight, width=0.2\textwidth, trim = 0 0 0 50, clip]{corn}
\includegraphics[height=0.2\textheight, width=0.2\textwidth, trim = 0 0 250 0, clip]{hogs}
\includegraphics[height=0.2\textheight, width=0.2\textwidth]{eggs}
\includegraphics[height=0.2\textheight, width=0.2\textwidth]{soybeans}
\includegraphics[height=0.2\textheight, width=0.2\textwidth, trim = 0 100 0 100, clip]{cows}
...
\begin{frame}
  ...
  \begin{itemize}
    \item Reduce to a relatively small number of inputs to guide field scientists.
    \item Better understand computer model deficiencies: does APSIM emphasize on soil too much?
    \item Model calibration: APSIM assumes a flat, uniform soil. TWI.
    \item Uncertainty quantification: APSIM is deterministic
    \item Provide on-line prediction for the final user.
    \item Inform the development \& calibration of new crop modules (e.g. cover crops).
    \item Assist agronomists in studying agronomic hypothesis.
  \end{itemize}

  \begin{alertblock}{Conclusion}
    Goals need to be more precise -- overlap with research questions?
  \end{alertblock}
\end{frame}
